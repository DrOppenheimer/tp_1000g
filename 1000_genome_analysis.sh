...
# Adapted from git/CDIS_GEUVADIS_analysis/download_and_stats_FASTQ_gamma.sh



# Parse input options
while getopts ":l:f:k:o:s:t:p:d:adbixuczh" opt; do ##### <- edit this (check all the options and their defaults)
    case $opt in
	l)
	...
	h)
	    ...
	    echo "     -l|--list          (string) Required - filename of list that contains the ARK ids";
	    echo "                                 Default = \"$LIST\"";
	    echo "                                  Lines that start with a \"#\" will be skipped"

	    echo "     -d|--download-pattern      (string) Required - url prefix for ARK mediated download ( \"https://s3\" or \"https://griffin\" )";
	    echo "                                 Default = \"\"";
	    # assume that data are available without credentials for now
	    
	    echo "     -h|--upload-host      (string) Required - url prefix for ARK download ( \"https://s3\" or \"https://griffin\" )";
	    echo "                                 Default = \"\"";
...
# Download data (with parcel) -- tracking the dl time ---- with and without parcel for all? (yes)

# process the file with Shenglai's Mutect docker (2 stage analysis) --- track processing time for each stage

# upload results and check (md5 check) -- tracking ul time (with and without parcel)

# create ARK id and log
...
do
	...
	if [[ $USEPARCEL -eq 1 ]]; then
		...
		do
			...
			echo -e "download_time:\t"$ELAPSED_TIME >> $my_run_log;
		...
		do
			...
			FINISH_TIME=`date +%s.%N`;
			ELAPSED_TIME=`echo "$FINISH_TIME - $START_TIME" | bc -l`;
			FILESIZE=`ls -ltr $mate_2 | cut -d " " -f 7`;
			echo -e "file_size:\t"$FILESIZE;
		    fi
		done
		echo `date`                              >> $my_run_log;
	    ...
	    echo "downloading $mate_2 withOUT parcel" >> $my_run_log;
