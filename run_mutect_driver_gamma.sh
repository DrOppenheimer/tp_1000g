...
PARCELIP='172.16.128.7';        # -rp # for external access (e.g. griff to Taiwan)
           #172.16.128.7
#PARCELIP='192.170.228.105';     # for internal access (e.g. griff to PDC)
...
# clear vars that should only by populated from argument parsing
if [[ -n $LIST ]]; then
    unset LIST
fi
if [[ -n $STRING ]]; then
...
while getopts ":l:cdzh" opt; do
	...
	h)
	    ...
	    echo "Revision gamma - can use a list (-l) or string (-s) as input"
	    echo ""
	    echo "USAGE (for list input)";
	    echo "     run_mutect_driver.sh -l <filename> [other options]";
	    echo "";
	    echo "EXAMPLE:";
	    echo "     run_mutect_driver.sh -l some_list.txt";
	    echo ""
	    echo "USAGE (for string input)";
	    echo "     run_mutect_driver.sh -s <url_string> [other options]";
	    echo "";
	    echo "EXAMPLE:";
	    echo "     run_mutect_driver.sh -s \" https://signpost.opensciencedatacloud.org/alias/ark:/31807/DC2-e0d641b2-a22e-421a-bb5d-e3e1bf6e3a92\" ";
	    echo ""
...
if [[ -n $LIST ]]; then # hande input if input is a list of ids 
	...
	# S3_W_PARCELDLSTARTTIME=$SECONDS;
	...
	# S3_WO_PARCEL_DL_STARTTIME=$SECONDS;
	...
	if [ $DOWNLOADONLY -eq 0 ]; then
	    ...
	    MESSAGE="Applying mutect to "$FILE;
	    echo $MESSAGE;
	    echo $MESSAGE >> $LOG;
...
if [[ -n $STRING ]]; then # hande input if input is a string 
